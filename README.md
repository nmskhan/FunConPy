...
You can choose one of two methods to perform skull stripping using the option ```--skullstrip=bet/afni```:
- BET (default): 
...
- AFNI:
  ...
  - The T1 image is is skull stripped with 3dSkullStrip, which is a modified version of the BET algorithm. The shrink factor, which controls the intensity threshold can be set with ```--anatfval```. Range is 0-1, and smaller values give larger brain outline estimates.
